...
exports.edit_comment = async function (req, resp) {
    if (req.session.userSessionId) {
        ...
        if (commentDoc.author.toString() === userDoc._id.toString()) {
            ...
            // update edit to db
            var updatedCommentDoc = await commentClassInstance.editComment(commentId, commentBody);
            (updatedCommentDoc) ? (resp.status(200).send("edit comment success!")) : (resp.status(400).send("edit comment failed"))
