...
const { UserClass } = require('../modules/modules');
...
exports.login_page = async function (req, resp) {
  ...
  if (req.session.userSessionId) {
    ...
    userDoc = await userClassInstance.getUserProfileBySession(
    ...
    if (userDoc) {
      ...
      try {
        ...
        resp.send({
          success: false,
          message:
            'An error occured while sending you the email. Please try again.',
        });
      }
      ...
      (async () => {
        try {
          ...
          resp.send({
            success: true,
            message:
              'Success! A password reset email has been sent to ' +
              req.body.email +
              '.',
          });
        } catch (err) {
  ...
  if (userDoc instanceof UserSchema && userDoc) {
    ...
    try {
      ...
      resp.send({
        ...
        message:
          'This link is no longer valid. Click the button below to generate a new link.',
      });
    }
  } else {
    resp.send({
      passwordChanged: false,
      message:
        'An error occured while trying to change your password. Click the button below to generate a new link.',
    });
  }
};
